...
def get_custom_vgg(archive_path=None, nout=10):
    ...
    layers = [
        # Initially Convs are: 128, 256, 512 Affines: 4096, 4096
        Conv((3, 3, 32), init=conv_init, bias=Constant(0), activation=Rectlin(), pad=1, batch_norm=True),
        Conv((3, 3, 32), init=conv_init, bias=Constant(0), activation=Rectlin(), pad=1, batch_norm=True),
        Pooling((2, 2), strides=2, op='max'),

        Conv((3, 3, 64), init=conv_init, bias=Constant(0), activation=Rectlin(), pad=1, batch_norm=True),
        Conv((3, 3, 64), init=conv_init, bias=Constant(0), activation=Rectlin(), pad=1, batch_norm=True),
        Pooling((2, 2), strides=2, op='max'),
        ...
        Affine(nout=1024, init=fc_init, bias=Constant(1), activation=Rectlin(), batch_norm=True),
        Dropout(0.5),
        Affine(nout=1024, init=fc_init, bias=Constant(1), activation=Rectlin(), batch_norm=True),
        Dropout(0.5),
